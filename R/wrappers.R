...
#' @export
getFeedingEdges <- function(dataset, mask, roostPolygons, roostBuffer = 50, inMaskThreshold = 0.33, consecThreshold = 2, distThreshold = 50, speedThreshUpper = 5, speedThreshLower = NULL, crs = "WGS84", longCol = "location_long.1", latCol = "location_lat.1", dateCol = "dateOnly"){
  # Argument checks
...
#' @return An edge list containing the following columns: `timegroup` gives the numeric index of the timegroup during which the interaction takes place. `minTimestamp` and `maxTimestamp` give the beginning and end times of that timegroup. `ID1` is the trackID of the first individual in this edge, and `ID2` is the trackID of the second individual in this edge.
#' @export
getFlightEdges <- function(dataset, mask, roostPolygons, roostBuffer = 50, inMaskThreshold = 0.33, consecThreshold = 2, distThreshold = 1000, speedThreshUpper = NULL, speedThreshLower = 5, crs = "WGS84", longCol = "location_long.1", latCol = "location_lat.1", dateCol = "dateOnly"){
  # Argument checks
  checkmate::assertClass(mask, "sf")
  checkmate::assertDataFrame(dataset)
  checkmate::assertClass(roostPolygons, "sf")
